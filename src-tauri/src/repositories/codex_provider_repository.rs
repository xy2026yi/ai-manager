...
impl CodexProviderRepository {
    /// 创建新的Codex供应商Repository实例
    ...
    pub async fn create_codex_provider(
        ...
        let query = r#"
            INSERT INTO codex_providers (
                name, url, token, type, enabled, created_at, updated_at
    ...
    /// 获取Codex供应商列表（解密token）
    pub async fn list_codex_providers_decrypted(&self) -> RepositoryResult<Vec<CodexProvider>> {
        let providers = self.list_all::<CodexProvider>().await?;
    ...
    /// 获取活跃的Codex供应商
    pub async fn list_active_providers(&self) -> RepositoryResult<Vec<CodexProvider>> {
        let query = "SELECT * FROM codex_providers WHERE enabled = 1 ORDER BY id DESC";
...
impl BaseRepository for CodexProviderRepository {
    ...
    async fn delete(&self, id: i64) -> RepositoryResult<bool> {
        let query = "DELETE FROM codex_providers WHERE id = ?";
    ...
    where
        T: for<'r> FromRow<'r, sqlx::sqlite::SqliteRow> + Send + Unpin,
    {
        let query = "SELECT * FROM codex_providers ORDER BY id DESC";
        ...
        T: for<'r> FromRow<'r, sqlx::sqlite::SqliteRow> + Send + Unpin,
    {
        ...
        let query = if where_conditions.is_empty() {
            "SELECT * FROM codex_providers ORDER BY id DESC LIMIT ?".to_string()
        } else {
            format!(
                "SELECT * FROM codex_providers WHERE {} ORDER BY id DESC LIMIT ?",
    ...
    async fn count(&self) -> RepositoryResult<i64> {
        let query = "SELECT COUNT(*) FROM codex_providers";
...
mod tests {
    ...
    async fn create_test_repository() -> CodexProviderRepository {
        let temp_dir = tempdir().unwrap();
        let db_path = temp_dir.path().join("test_codex.db");
    ...
    #[tokio::test]
    async fn test_codex_provider_crud() {
        ...
        let id = repo.create_codex_provider(&create_request).await.unwrap();
        assert!(id > 0);
        ...
        assert_eq!(provider.token, "sk-test-token");

        // 测试更新
        let update_request = UpdateCodexProviderRequest {
            name: Some("更新后的Codex".to_string()),
            url: None,
            token: None,
            r#type: None,
            enabled: Some(0),
        ...
        let updated = repo.update_codex_provider(id, &update_request).await.unwrap();
        assert!(updated);
        ...
        assert_eq!(updated_provider.name, "更新后的Codex");
