...
impl ApiServer {
    ...
    pub async fn with_config(config: ApiServerConfig) -> Result<Self, Box<dyn std::error::Error>> {
        ...
        let crypto_service = Arc::new(CryptoService::new("T4jCbDRQ6Z10_dzcJlhvyn2EfK-tTS4-dbpf27Lc1k8=")?);
    ...
    pub async fn run(self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        info!("📍 监听地址: http://{}", addr);
        info!("🔧 CORS支持: {}", if self.config.enable_cors { "启用" } else { "禁用" });
        info!("📊 追踪日志: {}", if self.config.enable_tracing { "启用" } else { "禁用" });
...
pub async fn handle_500(error: axum::BoxError) -> impl IntoResponse {
    ...
    ApiError::InternalServerError("服务器内部错误".to_string())
}