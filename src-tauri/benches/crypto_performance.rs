...
fn create_crypto_service() -> CryptoService {
    CryptoService::new("benchmark_key_for_crypto_performance_tests")
...
fn bench_batch_decryption(c: &mut Criterion) {
    ...
    let encrypted_list: Result<Vec<_>, _> =
        test_data_list.iter().map(|data| crypto_service.encrypt(data)).collect();
    ...
    c.bench_function("concurrent_encryption", |b| {
        b.to_async(&rt).iter(|| async {
            ...
            for i in 0..20 {
                ...
                let handle = tokio::task::spawn_blocking(move || service.encrypt(&data));
                handles.push(handle);
            }
...
fn bench_different_key_sizes(c: &mut Criterion) {
    let short_key_service =
        CryptoService::new("short").expect("Failed to create short key service");
    let medium_key_service =
        CryptoService::new("medium_key_for_testing").expect("Failed to create medium key service");
    let long_key_service = CryptoService::new("this_is_a_very_long_key_for_testing_purposes")
        .expect("Failed to create long key service");
