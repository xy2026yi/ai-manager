...
async fn test_claude_provider_complete_workflow() {
    ...
    assert_eq!(update_data["data"]["name"].as_str().unwrap(), "集成测试供应商-已更新");
    assert_eq!(update_data["data"]["timeout"].as_i64().unwrap(), 45000);
    assert_eq!(update_data["data"]["auto_update"].as_i64().unwrap(), 0);

    // 6. 测试搜索功能
    let search_response = client
        .get(&format!("{}/claude-providers?search=集成测试&limit=10", base_url))
        .send()
        .await
        .expect("搜索供应商请求失败");
    ...
    assert_eq!(search_results[0]["name"].as_str().unwrap(), "集成测试供应商-已更新");

    // 7. 测试连接测试
    let test_response = client
        .get(&format!("{}/claude-providers/{}/test", base_url, provider_id))
        .send()
        .await
        .expect("连接测试请求失败");
    ...
    let disable_response = client
        .post(&format!("{}/claude-providers/{}/disable", base_url, provider_id))
        .send()
        .await
        .expect("禁用供应商请求失败");
    ...
    let disabled_data: Value = get_disabled_response.json().await.expect("解析已禁用供应商响应失败");
    assert_eq!(disabled_data["data"]["enabled"].as_i64().unwrap(), 0);

    // 12. 重新启用供应商
    let enable_response = client
        .post(&format!("{}/claude-providers/{}/enable", base_url, provider_id))
        .send()
        .await
        .expect("启用供应商请求失败");
