...
use migration_ai_manager::migration_tool::{DataMigrationTool, PythonExportData};
use migration_ai_manager::models::*;
use serde_json;
use sqlx;
use std::fs;
use std::path::Path;
...
async fn test_full_migration_roundtrip() {
    ...
    // 2. 导入数据
    let import_report = setup.migration_tool
    ...
    // 3. 导出数据
    let exported_data = setup.migration_tool
        .export_to_json()
        .await
        .expect("数据导出应该成功");
    ...
    // 4. 验证数据完整性
    let original_data: PythonExportData = serde_json::from_str(&json_content)
        .expect("原始数据解析应该成功");
...
async fn test_encrypted_data_migration() {
    ...
    for provider in &mut test_data.claude_providers {
        provider.token = crypto_service.encrypt(&provider.token)
            .expect("token加密应该成功");
    }
    ...
    for provider in &exported_data.claude_providers {
        assert!(!provider.token.starts_with("gAAAA"), 
                "token应该被解密，当前仍为加密状态: {}", &provider.token[..20]);
    }
...
async fn test_database_schema_compatibility() {
    ...
    let _import_report = setup.migration_tool
        .import_from_json(&json_content)
        .await
        .expect("数据导入应该成功");
    ...
    assert_eq!(claude_count, test_data.claude_providers.len() as i64, 
               "Claude供应商数量应该匹配");
    ...
    assert_eq!(codex_count, test_data.codex_providers.len() as i64, 
               "Codex供应商数量应该匹配");
    ...
    assert_eq!(agent_count, test_data.agent_guides.len() as i64, 
               "Agent指导文件数量应该匹配");
    ...
    assert_eq!(mcp_count, test_data.mcp_servers.len() as i64, 
               "MCP服务器数量应该匹配");
    ...
    assert_eq!(config_count, test_data.common_configs.len() as i64, 
               "通用配置数量应该匹配");
...
async fn test_data_validation_rules() {
    ...
    // 添加重复的启用供应商
    test_data.claude_providers.push(migration_ai_manager::migration_tool::PythonClaudeProvider {
        id: None,
    ...
    // 检查是否有关于重复启用供应商的警告
    let has_duplicate_warning = import_report.warnings
        .iter()
...
fn create_sample_json_data() -> PythonExportData {
    PythonExportData {
        ...
        claude_providers: vec![
            migration_ai_manager::migration_tool::PythonClaudeProvider {
                id: None,
        ...
        codex_providers: vec![
            migration_ai_manager::migration_tool::PythonCodexProvider {
                id: None,
        ...
        agent_guides: vec![
            migration_ai_manager::migration_tool::PythonAgentGuide {
                id: None,
                name: "测试助手".to_string(),
        ...
        mcp_servers: vec![
            migration_ai_manager::migration_tool::PythonMcpServer {
                id: None,
                name: "test-filesystem".to_string(),
                r#type: Some("stdio".to_string()),
                timeout: Some(30000),
                command: "npx".to_string(),
                args: vec!["@modelcontextprotocol/server-filesystem", "/tmp".to_string()],
        ...
        common_configs: vec![
            migration_ai_manager::migration_tool::PythonCommonConfig {
                id: None,
...
async fn create_test_sample_file(temp_dir: &tempfile::TempDir) {
    ...
    fs::write(sample_file_path, json_content)
        .expect("测试样本文件写入应该成功");
}
...
fn verify_data_integrity(original: &PythonExportData, exported: &PythonExportData) {
    ...
    // 验证数量匹配
    assert_eq!(original.claude_providers.len(), exported.claude_providers.len(), 
               "Claude供应商数量应该匹配");
    assert_eq!(original.codex_providers.len(), exported.codex_providers.len(), 
               "Codex供应商数量应该匹配");
    assert_eq!(original.agent_guides.len(), exported.agent_guides.len(), 
               "Agent指导文件数量应该匹配");
    assert_eq!(original.mcp_servers.len(), exported.mcp_servers.len(), 
               "MCP服务器数量应该匹配");
    assert_eq!(original.common_configs.len(), exported.common_configs.len(), 
               "通用配置数量应该匹配");
    ...
    for (i, orig_provider) in original.claude_providers.iter().enumerate() {
        ...
        assert_eq!(orig_provider.token, exp_provider.token, "供应商token应该匹配");
    }
