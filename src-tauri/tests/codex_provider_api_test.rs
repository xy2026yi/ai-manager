...
#[tokio::test]
async fn test_codex_provider_complete_workflow() {
    ...
    let list_response = client
        .get(&format!("{}/codex-providers", base_url))
        .send()
        .await
        .expect("获取供应商列表请求失败");
    ...
    let create_request = json!({
        ...
        "token": "sk-integration-test-codex-key",
        "type": "gpt-4"
    });

    let create_response = client
        .post(&format!("{}/codex-providers", base_url))
        .json(&create_request)
        .send()
        .await
        .expect("创建供应商请求失败");
    ...
    let get_response = client
        .get(&format!("{}/codex-providers/{}", base_url, provider_id))
        .send()
        .await
        .expect("获取供应商详情请求失败");
    ...
    let update_request = json!({
        "name": "集成测试Codex供应商-已更新",
        "url": "https://api.openai.com/v1/updated",
        "token": "sk-updated-test-key"
    });

    let update_response = client
        .put(&format!("{}/codex-providers/{}", base_url, provider_id))
        .json(&update_request)
        .send()
        .await
        .expect("更新供应商请求失败");
    ...
    let stats_response = client
        .get(&format!("{}/codex-providers/stats", base_url))
        .send()
        .await
        .expect("获取统计信息请求失败");
    ...
    let delete_response = client
        .delete(&format!("{}/codex-providers/{}", base_url, provider_id))
        .send()
        .await
        .expect("删除供应商请求失败");
...
#[tokio::test]
async fn test_codex_provider_validation_errors() {
    let base_url = "http://localhost:8080/api/v1";
    let client = reqwest::Client::new();
...
#[tokio::test]
async fn test_codex_provider_not_found_errors() {
    let base_url = "http://localhost:8080/api/v1";
    let client = reqwest::Client::new();

    // 测试获取不存在的供应商
    let response = client
        .get(&format!("{}/codex-providers/99999", base_url))
        .send()
        .await
        .expect("不存在供应商请求失败");
    ...
    assert!(!data["success"].as_bool().unwrap());
    assert!(data["error"]["message"].as_str().unwrap().contains("Codex供应商不存在"));
    ...
    let response = client
        .put(&format!("{}/codex-providers/99999", base_url))
        .json(&update_request)
        .send()
        .await
        .expect("更新不存在供应商请求失败");
    ...
    // 测试删除不存在的供应商
    let response = client
        .delete(&format!("{}/codex-providers/99999", base_url))
        .send()
        .await
        .expect("删除不存在供应商请求失败");
