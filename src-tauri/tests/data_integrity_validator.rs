...
impl DataIntegrityValidator {
    ...
    /// 执行完整的数据完整性校验
    pub async fn validate_full_integrity(
        &self,
        original_data: &PythonExportData,
    ) -> IntegrityReport {
    ...
    /// 校验数据记录数量
    async fn validate_record_counts(
        &self,
        original_data: &PythonExportData,
        ...
        self.validate_count(
            "Claude供应商",
            claude_count,
            original_data.claude_providers.len(),
            report,
        )
        .await;
        ...
        self.validate_count(
            "Codex供应商",
            codex_count,
            original_data.codex_providers.len(),
            report,
        )
        .await;
        ...
        self.validate_count(
            "Agent指导文件",
            agent_count,
            original_data.agent_guides.len(),
            report,
        )
        .await;
        ...
        self.validate_count(
            "通用配置",
            config_count,
            original_data.common_configs.len(),
            report,
        )
        .await;
    }
        ...
        match actual_count {
            Some(actual) => {
                if actual as usize == expected_count {
                    ...
                    let msg = format!(
                        "❌ {} 数量不匹配: 实际={}, 期望={}",
                        entity_name, actual, expected_count
                    );
                    error!("{}", msg);
                    report.errors.push(msg);
    ...
    /// 校验数据内容完整性
    async fn validate_data_content(
        &self,
        original_data: &PythonExportData,
        report: &mut IntegrityReport,
    ) {
        ...
        // 校验Claude供应商内容
        self.validate_claude_providers_content(&original_data.claude_providers, report)
            .await;

        // 校验Codex供应商内容
        self.validate_codex_providers_content(&original_data.codex_providers, report)
            .await;
        ...
        // 校验通用配置内容
        self.validate_common_configs_content(&original_data.common_configs, report)
            .await;
    }

    /// 校验Claude供应商内容
    async fn validate_claude_providers_content(
        &self,
        original_providers: &[migration_ai_manager_lib::migration_tool::PythonClaudeProvider],
        report: &mut IntegrityReport,
    ) {
        let pool = self.db_manager.pool();

        match sqlx::query("SELECT * FROM claude_providers ORDER BY id").fetch_all(pool).await {
            Ok(rows) => {
                if rows.len() == original_providers.len() {
                    ...
                    for (i, row) in rows.iter().enumerate() {
                        if i < original_providers.len() {
                            ...
                            if db_name == original.name {
                                ...
                                let msg = format!(
                                    "Claude供应商名称不匹配: 数据库='{}', 原始='{}'",
                                    db_name, original.name
                                );
                    ...
                    let msg = format!(
                        "❌ Claude供应商内容记录数不匹配: 数据库={}, 原始={}",
                        rows.len(),
                        original_providers.len()
                    );
                    error!("{}", msg);
                    report.errors.push(msg);
                    report.failed_checks += 1;
    ...
    /// 校验Codex供应商内容
    async fn validate_codex_providers_content(
        &self,
        original_providers: &[migration_ai_manager_lib::migration_tool::PythonCodexProvider],
        report: &mut IntegrityReport,
    ) {
        let pool = self.db_manager.pool();

        match sqlx::query("SELECT * FROM codex_providers ORDER BY id").fetch_all(pool).await {
            Ok(rows) => {
                if rows.len() == original_providers.len() {
    ...
    /// 校验Agent指导文件内容
    async fn validate_agent_guides_content(
        &self,
        original_guides: &[migration_ai_manager_lib::migration_tool::PythonAgentGuide],
        report: &mut IntegrityReport,
    ) {
        let pool = self.db_manager.pool();

        match sqlx::query("SELECT * FROM agent_guides ORDER BY id").fetch_all(pool).await {
            Ok(rows) => {
    ...
    /// 校验MCP服务器内容
    async fn validate_mcp_servers_content(
        &self,
        original_servers: &[migration_ai_manager_lib::migration_tool::PythonMcpServer],
        report: &mut IntegrityReport,
    ) {
        let pool = self.db_manager.pool();

        match sqlx::query("SELECT * FROM mcp_servers ORDER BY id").fetch_all(pool).await {
            Ok(rows) => {
    ...
    /// 校验通用配置内容
    async fn validate_common_configs_content(
        &self,
        original_configs: &[migration_ai_manager_lib::migration_tool::PythonCommonConfig],
        report: &mut IntegrityReport,
    ) {
        let pool = self.db_manager.pool();

        match sqlx::query("SELECT * FROM common_configs ORDER BY id").fetch_all(pool).await {
            Ok(rows) => {
            ...
            .await
        {
            Ok(rows) => {
                ...
                if encrypted_count == total_count && total_count > 0 {
                    info!(
                        "✅ 所有Claude供应商token都已正确加密: {}/{}",
                        encrypted_count, total_count
                    );
                    report.passed_checks += 1;
