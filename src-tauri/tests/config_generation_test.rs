...
impl ConfigGeneratorTester {
    ...
    /// 测试Claude配置文件生成
    pub async fn test_claude_config_generation(
        &self,
    ) -> Result<ClaudeConfigTestResult, Box<dyn std::error::Error>> {
        info!("测试Claude配置文件生成...");
        ...
        if let Some(config_obj) = loaded_config.as_object() {
            ...
            // 验证基本结构
            if config_obj.contains_key("claude_api_key") && config_obj.contains_key("models") {
    ...
    /// 测试Codex配置文件生成
    pub async fn test_codex_config_generation(
        &self,
    ) -> Result<CodexConfigTestResult, Box<dyn std::error::Error>> {
        info!("测试Codex配置文件生成...");
        ...
        let auth_config: serde_json::Value =
            serde_json::from_str(&fs::read_to_string(&auth_path)?)?;
    ...
    pub async fn test_python_compatibility(&self) -> Result<bool, Box<dyn std::error::Error>> {
        ...
        let codex_auth: serde_json::Value =
            serde_json::from_str(&fs::read_to_string(&codex_auth_path)?)?;
        ...
        let codex_config_valid =
            codex_config_content.contains("[openai]") && codex_config_content.contains("api_key");
