...
impl DataIntegrityValidator {
    async fn new(original_db_path: &str, migrated_db_path: &str) -> Result<Self, Box<dyn std::error::Error>> {
    ...
    // 验证表结构一致性
    async fn verify_table_schemas(&self) -> Result<Vec<TestDataRecord>, Box<dyn std::error::Error>> {
        let tables = vec![
            "claude_providers",
            "codex_providers", 
            "agent_guides",
    ...
    // 验证单个表的Schema
    async fn verify_single_table_schema(&self, table_name: &str) -> Result<TestDataRecord, Box<dyn std::error::Error>> {
    ...
    // 获取表结构信息
    async fn get_table_schema(&self, pool: &SqlitePool, table_name: &str) -> Result<HashMap<String, String>, Box<dyn std::error::Error>> {
        let query = format!(
            "PRAGMA table_info({})",
            table_name
        );
    ...
    // 获取表的行数
    async fn get_row_count(&self, pool: &SqlitePool, table_name: &str) -> Result<i64, Box<dyn std::error::Error>> {
    ...
    // 验证数据内容一致性（非加密字段）
    async fn verify_unencrypted_data(&self) -> Result<Vec<TestDataRecord>, Box<dyn std::error::Error>> {
        let mut results = Vec::new();
    ...
    async fn verify_claude_providers(&self) -> Result<TestDataRecord, Box<dyn std::error::Error>> {
        ...
        if original_rows.len() != migrated_rows.len() {
            integrity_issues.push(format!(
                ...
                original_rows.len(), migrated_rows.len()
            ));
        }
        ...
        for (i, (orig_row, mig_row)) in original_rows.iter().zip(migrated_rows.iter()).enumerate() {
            ...
            let fields_to_check = vec![
                "name", "url", "max_tokens", "temperature", "model", 
    ...
    async fn verify_agent_guides(&self) -> Result<TestDataRecord, Box<dyn std::error::Error>> {
        let query = "SELECT id, name, description, created_at, updated_at FROM agent_guides ORDER BY id";
    ...
    // 生成数据完整性报告
    fn generate_integrity_report(&self, schema_results: Vec<TestDataRecord>, count_results: Vec<TestDataRecord>, data_results: Vec<TestDataRecord>) -> String {
        ...
        // 总体评估
        let total_issues = schema_results.iter().map(|r| r.mismatched_fields.len())
        ...
        if total_issues == 0 {
            ...
            report.push_str(&format!("⚠️ **发现问题**: 共发现 {} 个问题，需要修复后才能完成迁移。\n", total_issues));
        }
...
mod tests {
    ...
    // 创建测试用的原始数据库
    async fn create_test_original_database(db_path: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    async fn test_data_integrity_validation() {
        ...
        // 检查Claude供应商表的验证结果
        let claude_result = data_results.iter().find(|r| r.table_name == "claude_providers").unwrap();
